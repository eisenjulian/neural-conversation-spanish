...
tf.app.flags.DEFINE_string("data_dir", "data/subs/", "Data directory")
...
tf.app.flags.DEFINE_integer("steps_per_print", 1,
                            "How many training steps to do to print.")
tf.app.flags.DEFINE_integer("steps_per_checkpoint", 5, # 200
    ...
    while source and target and (not max_size or counter < max_size):
      ...
      for bucket_id, (source_size, target_size) in enumerate(_buckets):
        if len(source_ids) < source_size and len(target_ids) < target_size:
          ...
          break
      source, target = target, source_file.readline()
  return data_set
...
def train():
  ...
  train, _ = utils.prepare_data(FLAGS.data_dir, FLAGS.vocab_size)
  ...
  with tf.Session() as sess:
    ...
    while True:
      ...
      step_time += (time.time() - start_time) / FLAGS.steps_per_print
      loss += step_loss / FLAGS.steps_per_print
      current_step += 1
...
def decode():
  with tf.Session() as sess:
    ...
    while sentence:
      ...
      sentence_tokens = utils.basic_tokenizer(tf.compat.as_bytes(sentence))
